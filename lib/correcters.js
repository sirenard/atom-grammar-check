    ...
    await gingerbread(text, (error, text, result, corrections) => {
        if(!error) {
            for (let correction of corrections) {
                res.push(makeCorrection(correction.start, correction.length, correction.correct.length?[correction.correct]: []));
            }
        }
        releaseGinger();
    });
...
export async function correctLanguageTool(text, args){
    let res = [];
    let url = new URL(new URL(args.url || 'http://localhost:8081/') + "v2/check");
    ...
    if(queryResult.matches){
        ...
        queryResult.matches.map(match =>{
            res.push(makeCorrection(match.offset, match.length, match.replacements.map(v => v.value)));
        });
    }
...
function makeCorrection(start, length, replacements) {
    return {
        start: start,
        length: length,
        replacements: replacements.splice(0,5) //max 5 possible replacements to avoid spamming
    };
}